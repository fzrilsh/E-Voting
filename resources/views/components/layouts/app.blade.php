...
<body>
    <nav class="bg-white fixed top-0 left-0 w-full m-auto z-50" x-data="{ isOpen: false }">
            ...
            <div class="mx-auto flex items-center justify-between">
                ...
                <div class="hidden md:block">
                    <div class="ml-4 flex items-center md:ml-6">
                        ...
                        @if (auth()->check())
                            <div>
                                <button @click="isOpen = !isOpen" type="button" class="relative flex max-w-xs items-center rounded-full text-sm focus:outline-none focus:ring-2 focus:ring-gray-800 focus:ring-offset-2 focus:ring-offset-gray-800" id="user-menu-button" aria-expanded="false"
                                    aria-haspopup="true">
                        ...
                        @else
                            <div class="relative ml-3">
                                <a href="{{ route('login') }}" class="px-4 py-2 text-sm font-medium text-white bg-red-telkom rounded-md hover:ring-offset-2 hover:ring-offset-red-telkom hover:bg-red-telkom-hover">Login</a>
                            </div>
                        @endif

                        <div class="relative ml-3">
                            <div x-show="isOpen" x-transition:enter="transition ease-out duration-100 transform" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100" x-transition:leave="transition ease-in duration-75 transform"
                                x-transition:leave-start="opacity-100 scale-100" x-transition:leave-end="opacity-0 scale-95" class="absolute right-0 z-10 mt-2 w-48 origin-top-right rounded-md bg-white py-1 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none" role="menu"
                ...
                <div class="-mr-2 flex md:hidden">
                    <!-- Mobile menu button -->
                    <button type="button" @click="isOpen = !isOpen" class="relative inline-flex items-center justify-center rounded-md bg-gray-200 p-2 text-gray-800 hover:bg-gray-300 hover:text-gray-800 focus:outline-none focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-gray-300"
        ...
        <div x-show="isOpen" x-transition:enter="transition ease-in-out duration-100 transform" x-transition:enter-start="scale-95" x-transition:enter-end="opacity-100 scale-100" x-transition:leave="transition ease-in duration-75 transform" x-transition:leave-start="opacity-100 scale-100"
            x-transition:leave-end="opacity-0 scale-95" class="md:hidden" id="mobile-menu">
            <div class="space-y-1 px-2 pb-3 pt-2 sm:px-3">
                <!-- Current: "bg-gray-900 text-white", Default: "text-gray-300 hover:bg-gray-700 hover:text-white" -->
                <a href="{{ route('dashboard') }}" class="block rounded-md px-3 py-2 text-base font-medium text-gray-600 hover:bg-gray-200" aria-current="page">Dashboard</a>
                <a href="{{ route('vote') }}" class="block rounded-md px-3 py-2 text-base font-medium text-gray-600 hover:bg-gray-200">Vote</a>
                <a href="{{ route('candidates') }}" class="block rounded-md px-3 py-2 text-base font-medium text-gray-600 hover:bg-gray-200">Candidates</a>
                <a href="{{ route('quick-count') }}" class="block rounded-md px-3 py-2 text-base font-medium text-gray-600 hover:bg-gray-200">Quick Count</a>
            </div>
            <div class="border-t border-gray-700 pb-3 pt-4 px-2 sm:px-3">
                ...
                @else
                    <a href="{{ route('login') }}" class="block w-full px-4 py-2 font-medium text-white bg-red-telkom rounded-md hover:ring-offset-2 hover:ring-offset-red-telkom hover:bg-red-telkom-hover">Login</a>
                @endif
            </div>
        </div>
    ...
    <footer class="mt-10 grid max-sm:grid-cols-2 max-sm:grid-flow-dense w-full place-items-start gap-4 grid-flow-col bg-gray-100 text-base-content mx-auto py-10 max-w-7xl px-4 max-sm:px-10 lg:px-8">
