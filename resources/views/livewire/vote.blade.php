    ...
    @else
        <section class="mt-10 px-5 sm:px-6 lg:px-8 mx-auto max-w-7xl flex flex-col w-full h-auto">
            <form wire:submit="vote" method="POST" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 w-full mb-5 h-auto">
                ...
                @endforeach
                <div class="mt-8 grid-cols-1 md:grid-cols-2 lg:col-span-3 w-full flex items-center justify-center">
                    <button type="submit" class="px-4 py-2 bg-carmine-600 text-white font-semibold rounded-lg hover:bg-carmine-700 transition duration-200">Konfirmasi Pilihan</button>
                </div>
            </form>
        </section>
    @endif
